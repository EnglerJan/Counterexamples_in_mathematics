...
By definition, $f\colon \mathbb{R}\to \mathbb{R}$ is Lebesque measurable if it is measurable as a map $(\mathbb{R},\mathcal{L})\to (\mathbb{R}, \mathcal{B}).$ Let $\psi\colon [0,1]\to [0,2]$ be as in (\ref{cantor+x}), then since $[0,1]$ is compact $\psi^{-1}$ is also continuous (therefore Lebesque measurable). Denote $A\subset[0,1]$ a non-Borel set from (\ref{nonborel measurable set}), then since $\lambda(A)=0,\ \mathbb{1}_A\colon [0,1]\to \mathbb{R}$ is Lebesque measurable. But
\begin{equation*}
...
\begin{equation*}
	\nm{\delta_1}_{(C[0,1])^{*}}=1, \ C[0,1]<L^{\infty}(0,1) \Rightarrow \exists T\in (L^{\infty}(0,1))^{*}\colon T\rvert_{C[0,1]}=\delta_1, \ \nm{T}_{(L^{\infty}(0,1))^{*}}=1.
\end{equation*}
...
\begin{equation*}
	\forall x\in Y \ \exists! a\in A\colon \nm{x-a}=\inf_{b\in A}\nm{x-b} \ \ \ \hdots a:=P_A(x)
\end{equation*}
by the Projection theorem, every nonempty closed convex subset of a Hilbert space is proximital. We show that in $X=L^{1}$, $V:=\{f\in L^{1}(0,1)\colon \langle \phi, f\rangle=1\}$ is not proximital, even though it is a closed convex subset. We wish to project $0\in L^{1}$ to $V.$ Clearly
\begin{equation*}
	\forall \epsilon>0\ \exists f_\epsilon\in L^{1}\colon \nm{f_\epsilon}_{L^{1}}\leq 1+\epsilon , \ \langle \phi, f_\epsilon\rangle=1 \ \ \ \&\ \ \ \forall f\in L^{1}\colon \langle \phi, f\rangle=1 \Rightarrow \nm{f}_{L^{1}}\geq 1
\end{equation*}
\begin{equation*}
