...
\end{align*}
By Vitali's theorem, any Lebesque-measurable set of positive measure has a non-measurable subset. Denote $N\subset \psi(\mathcal{C})$ such a non-measurable subset. Then $\psi^{-1}(N)\subset \mathcal{C}$ is Lebesque-measurable since $\lambda(\mathcal{C})=0.$ But $\psi^{-1}$ is increasing, therefore Borel, so if $\psi^{-1}(N)$ was Borel-measurable, also
\begin{equation*}
  N = (\psi^{-1})^{-1}(\psi^{-1}(N))
\end{equation*}
would be, contradiction. Therefore $\psi^{-1}(N)$ is Lebesque-measurable but not Borel-measurable.
